...
<body>
                ...
                <nav class="navbar navbar-expand-lg bg-dark navbar-dark py-3 py-lg-0 px-0">
                    <div class="collapse navbar-collapse justify-content-between ml-5" id="navbarCollapse">
                        <div class="navbar-nav mr-auto py-0 text-center">
                            <a href="{{route('user#cate')}}" class="nav-item nav-link active">Home</a>
  ...
  <i class="fa-solid fa-user "></i> {{Auth::user()->name}}
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                  <a class="dropdown-item" href="{{route('user#details')}}"> <i class="fa-solid fa-user"></i> Account</a>
                                  <a class="dropdown-item" href="{{route('user#changePage')}}">Change password</a>
                                  <span class="dropdown-item">
                                    <form action="{{route('logout')}}" method="post" class="d-inline">
                                        @csrf
                                    <button class="btn btn-dark ml-2"><i class="fa-solid fa-right fa-right-from-bracket"></i> LogOut</button>

                                    </form>
                                  </span>
                                </div> --}}
                                <div class="dropdown " style="margin-right: 46px;">
                                    <button class="btn btn-secondary dropdown-toggle " type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            <i class="fa-solid fa-user"></i>  {{Auth::user()->name}}
                                    </button>
                                    <div class="dropdown-menu " aria-labelledby="dropdownMenuButton">
                                      <a class="dropdown-item" href="#"><i class="fa-solid fa-user"></i>  {{Auth::user()->name}}</a>
                                      <a class="dropdown-item" href="{{route('user#details')}}">Change password</a>
                                      <span class="dropdown-item">
                                        <form action="{{route('logout')}}" method="post" class="d-inline">
                                            @csrf
                                        <button class="btn btn-dark ml-2"><i class="fa-solid fa-right fa-right-from-bracket"></i> LogOut</button>
            ...
            <div class="col-lg-8 table-responsive mb-5">
                <table class="table table-light table-borderless table-hover text-center mb-0" id="table">
                    ...
                    <tbody class="align-middle">
                        ...
                        <tr>
                            <td class="align-middle"> <img style="width:50px;" src="{{ asset('storage/' . $cart->pizza_image) }}" alt="Image">{{$cart->pizza_name}}
                            <input type="hidden" id="productId" value="{{$cart->pizza_id}}">
                            <input type="hidden" name="" id="orderId" value="{{$cart->id}}">
                            <input type="hidden" id="userId" value="{{$cart->user_id}}">
                            </td>
                            <td class="align-middle" id="pizzaPrice">{{$cart->pizza_price}} kyats</td>
                            <input type="hidden" name="" value="{{$cart->pizza_price}}" id="pizzaprice">
                ...
                <div class="bg-light p-30 mb-5">
                    <div class="border-bottom pb-2">
                        <div class="d-flex justify-content-between mb-3">
                            <h6>Subtotal</h6>
                            <h6 id="finaltotal">{{$totalprice}} kyats</h6>
                        </div>
                        <div class="d-flex justify-content-between">
                            <h6 class="font-weight-medium">Delivery</h6>
                            <h6 class="font-weight-medium" id="Delivery">3000 kyats</h6>
                        </div>
                    </div>
                    <div class="pt-2">
    ...
    <div class="container-fluid bg-dark text-secondary mt-5 pt-5">
        <div class="row px-xl-5 pt-5">
            ...
            <div class="col-lg-8 col-md-12" id="row">
                <div class="row">
        ...
        $('#orderBtn').click(function(){
            ...
            $('#table tbody  tr').each(function(index,row){
               $orderList.push({
                ...
                'pizza_id':$(row).find('#productId').val(),
